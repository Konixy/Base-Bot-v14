...
export default {
  ...
  async execute(client: Client, Client: ClientType, message: Message) {
    ...
    if (command.permissions) {
      ...
      if (command.guildOwnerOnly) {
        if (
          ...
          return message
            .reply(
              "Vous devez être le propriétaire du serveur pour exécuter cette commande."
            )
            .catch(() => {});
      }
    ...
    console.log(
      `${`[CMD]`.blue} ${`@${message.author.tag}`.bgBlue.black} in ${
        message.guild.name.bgWhite.black
      } in ${`#${message.channel.name}`.bgWhite.black} : ${message.content}`
    );
  },
};
